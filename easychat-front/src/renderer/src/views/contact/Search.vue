<template>
    <ContentPanel>
        <div class="search-form">
            <el-input clearable placeholder="请输入用户ID或群组的ID" v-model="contactId" size="large" @keydown.enter="search">
            </el-input>
        ...
        <div v-if="searchResult && Object.keys(searchResult).length > 0" class="search-result-panel">
            <div class="search-result">
                ...
                <UserBaseInfo :userInfo="searchResult" :showArea="searchResult.contactType == 'USER' ? true : false "></UserBaseInfo>
            </div>
            <div class="op-btn">
                <el-button type="primary" 
                    v-if="searchResult.status == null ||
                ...
                <span v-if="searchResult.status == 5 || searchResult.status == 6" @click="sendFile">对方拉黑了你</span>
            </div>
        </div>
...
const searchAddRef = ref();//用来获取SearchAdd组件的实例
